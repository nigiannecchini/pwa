...
/**
 * Gets the weather forecast from the Geo Code API for the given location.
 *
 * @param {Request} req request object from Express.
 * @param {Response} resp response object from Express.
 */
function getCurrentForecast(req, resp) {
  const location = req.params.location || '-23.5045601,-46.851028299999996';
  const url = `${CURRENT_BASE_URL}?q=${location}&key=${CURRENT_API_KEY}`;
  fetch(url).then((resp) => {
    if (resp.status !== 200) {
      throw new Error(resp.statusText);
    }
...
function startServer() {
  ...
  app.get('/currentForecast/:location', getCurrentForecast);
