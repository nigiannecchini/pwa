...
function showPosition(position)
{
  ...
  var label = "";
  getCurrentForecastFromNetwork(geo).then((data) => {
      label = data.results[0].components.city;
      const location = {label: label, geo: geo};
      // Create a new card & get the weather data from the server
      const card = getForecastCard(location);
      getForecastFromNetwork(geo).then((forecast) => {
...
function renderForecast(card, data) {
  ...
  card.querySelector('.current .temperature .value')
      .textContent = Math.round(((data.currently.temperature)-32)*5/9);
  ...
  card.querySelector('.current .wind .value')
      .textContent = Math.round((data.currently.windSpeed*1.609));
  ...
  futureTiles.forEach((tile, index) => {
    ...
    tile.querySelector('.temp-high .value')
        .textContent = Math.round(((forecast.temperatureHigh)-32)*5/9);
    tile.querySelector('.temp-low .value')
        .textContent = Math.round(((forecast.temperatureLow)-32)*5/9);
  });
 ...
 * @return {Object} The weather forecast, if the request fails, return null.
 */
function getCurrentForecastFromNetwork(coords) {
  return fetch(`/currentForecast/${coords}`)
      .then((response) => {
        return response.json();
      })
      .catch(() => {
...
function init() {
  ...
  document.getElementById('butCurrentLocation').addEventListener('click', addCurrentLocation);
